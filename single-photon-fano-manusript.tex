...
\end{equation}
Here, the commutator in the first term at the right hand side describes the coherent evolution of the isolated $\mathcal{H}_{s}$.
The coupling with the environment of the system is taken into account by the rest terms. %\cite{PhysRevA.59.4756,PhysRevB.79.235325,PhysRevB.79.235326}.
\revision{The electrical pumping applied to the molecule p (m) is described by the second term, where an electron-hole can be injected from a nonequilibrium electronic reservoir $j$ ($j=1~(2)$ for molecule p (m)). 
...
The reverse process characterized by the third term describes the creation of an electron-hole pair in the electrical reservoir. The rates for these two processes are  $\Gamma^{+}_{\nu}=E_{\nu}f^{e}_{j}$ and $\Gamma^{-}_{\nu}=E_{\nu}(1-f^{e}_{j})$, respectively. Here $f_{j}^{e}$ is the distribution function of the electronic reservoir.} 
...
%
The photons emitted from molecules are collected by a photon bath, the system coupling to which is represented by $\gamma_{d\nu}^{-}=\gamma_{d\nu}[n_{B}(\omega_{\nu})+1]$, which describes photon emission to the bath induced by the inelastic transition from level $l~(e)$ to $h~(g)$. The absorption process corresponds to $\gamma_{d\nu}^{+}=\gamma_{d\nu}n_{B}(\omega_{\nu})$. This occurs when the temperature of the bath is large enough, such that it can emit thermal photons and the transition from $h~(g)$ to $l~(e)$ becomes possible.
%
Similarly, the coupling between the cavity mode and the photon bath is characterized by $\kappa_{c}^{-}=\gamma_{c}[n_{B}(\omega_{c})+1]$ and $\kappa_{c}^{+}=\gamma_{c}n_{B}(\omega_{c})$, respectively.
%
$n_{B}(\omega)=(e^{\hbar\omega/k_{B}T}-1)^{-1}$ is the Bose-Einstein distribution. 
...
Here, we consider $T\approx0$ , such that $\Gamma^{-}_{\nu}\approx 0$, $\gamma_{d\nu}^{+}\approx0$, and $\kappa_{p}^{+}\approx0$. In this case, the process of generating electron-hole pairs in \revision{nonequilibrium electronic reservoir} is prohibited.
Meanwhile, photon bath-induced excitations for the cavity mode and the molecular exciton vanish. \revision{The detailed descriptions for above discussions can be found in Section I of the Supplemental Material \cite{SupplementalMaterial}.}
